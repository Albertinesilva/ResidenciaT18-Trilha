package atvEmSala.exDegas.poligonosRuins.poligonos;
...
public class Poligono {
	...
	public void mostraPontos() {
		for (int i = 0; i < pontos.size(); i++) {
			System.out.println(
					"(" + pontos.get(i).getX() + "," + pontos.get(i).getY() + "," + pontos.get(i).getZ() + ")");
		}
	}
	...
	double perimetro() {
		...
		for (int i = 0; i < pontos.size() - 1; i++) {
			p1 = pontos.get(i);
			p2 = pontos.get(i + 1);
			per += Math.sqrt(Math.pow(p1.getX() - p2.getX(), 2) + Math.pow(p1.getY() - p2.getY(), 2)
